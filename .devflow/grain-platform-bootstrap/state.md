...
## 当前阶段
- Pause-ready after init-strategy closure and warehouse CRUD
...
## 下一步
- 优先补用户 CRUD。
- 待初始化策略和后台管理接口稳定后，再补 PowerShell 验收脚本，串起后端启动、固定模板下载、固定模板导入、旧 CSV / 旧行式 Excel 导入和首页概览检查。
...
## 最小活跃上下文摘要
- 当前已解除运行态阻塞：本机 MySQL `root` 密码已恢复为 `123456`，数据库优先 MVP 的首页概览、固定模板下载、固定模板导入、旧 CSV 和旧行式 `.xls` 兼容 smoke 已全部通过；同时本轮已完成初始化策略收口，默认启动不再自动清库，显式重置走 `scripts/reset-demo-db.ps1`。当前已完成数据主线 CRUD、后台页第一轮列表统一、数据管理页后端分页适配，以及仓库管理 CRUD；`WarehouseView.vue` 现在支持新增 / 编辑 / 删除，仓库接口已补齐 `PUT/DELETE`。当前过程记录已整体迁移到 `.devflow/grain-platform-bootstrap/`，并新增 `NEXT-SESSION-PROMPT.devflow.md` 作为新的恢复提示词副本。下一步应直接进入用户 CRUD，其后顺序为 PowerShell 验收脚本。
- 当前已解除运行态阻塞：本机 MySQL `root` 密码已恢复为 `123456`，数据库优先 MVP 的首页概览、固定模板下载、固定模板导入、旧 CSV 和旧行式 `.xls` 兼容 smoke 已全部通过；同时本轮已完成初始化策略收口，默认启动不再自动清库，显式重置走 `scripts/reset-demo-db.ps1`。当前已完成数据主线 CRUD、后台页第一轮列表统一、数据管理页后端分页适配，以及仓库管理 CRUD；`WarehouseView.vue` 已支持新增 / 编辑 / 删除，仓库接口已补齐 `PUT/DELETE`。本次会话中用户 CRUD 尚未开始实现代码，仅完成了读取和边界确认。最新恢复入口已更新为 `2026-04-09-019-pause-ready-after-init-and-warehouse-crud`，下次应直接进入用户 CRUD，实现完成后再补 PowerShell 验收脚本。
