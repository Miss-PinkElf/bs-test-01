...
本阶段聚焦正式 Vue 前端里“图表时间轴过长、x 轴展示不全”的问题，且只收口用户明确要求保留并增强的两张核心图：

1. `frontend/src/views/DataView.vue` 中的 **粮温汇总趋势图**
2. `frontend/src/views/PredictionView.vue` 中的 **实际值 / 预测值双线图**

目标是在**不删图、不改页面主结构、不扩到 `frontend-next`** 的前提下，让这两张图都支持用户选择时间范围，并通过合理的默认时间窗口与图表交互兜底，解决首屏拥挤、标签过长和展示不全问题。
...
- **D-01:** Phase 12 只覆盖用户明确指定的两张图：`DataView.vue` 的粮温汇总趋势图，以及 `PredictionView.vue` 的实际值 / 预测值双线图。
- **D-02:** 本阶段不扩展为“全站所有 ECharts 图统一治理”，也不改 `frontend-next`。
- **D-03:** `BigScreenView.vue` 不是本阶段必做范围；若后续需要统一图表轴策略，另开新 phase。

### 时间范围能力

- **D-04:** 上述两张图都必须支持用户主动选择时间范围，不能只靠标签旋转或缩写硬撑。
...
- 用户明确要求必须保留并增强两张图：粮温汇总趋势图，以及预测页实际值 / 预测值双线图。
- 用户明确要求两张图都支持用户选择时间范围。
- 用户接受推荐方案：范围收口到最直接受影响的正式页面；主要解法是“时间范围优先，图表交互兜底”；`PredictionView` 的时间范围只裁图、不改任务语义。
...
- `BigScreenView.vue` 四块图的统一 x 轴收口
