...
preset: none
created: 2026-04-11
reviewed_at: 2026-04-11T13:37:18.4601186+08:00
---
...
- `DashboardView` 列表卡与表格模块默认仍用 `16px` 间距，不允许因分页组件加入而压缩到 `8px` 以下。

Phase 9 收口约束：
- 列表顶部搜索/按钮工具条统一收口到 **8px** 的组件级间距与 **16px** 的区块级间距，不再保留任何 `12px` 旧值描述。
- 表格与分页器之间固定 `16px`。
...
- 视觉焦点固定为 **左侧用户主表**；顶部 summary cards 负责先给出全局数量概览，右侧角色说明只作为辅助理解，不得抢占主列表注意力。
...
- 视觉焦点固定为 **仓库主列表 + 右侧当前选中仓库详情** 这组“列表驱动详情”的组合；顶部说明卡只能作为进入页的上下文，不得压过详情卡和主表。
...
- 视觉焦点固定为 **顶部 overview 指标卡 + 下方三块真实数据模块**；次级说明卡永远是最弱权重模块，不得与三个分页列表争抢首屏注意力。
